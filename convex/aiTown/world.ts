...
export const serializedWorld = {
  ...
  currentArticleSummary: v.optional(v.string()),
  publicStatements: v.array(v.object({ agentName: v.string(), statement: v.string(), createdAt: v.number() })),
};
...
export class World {
  ...
  constructor(serialized: SerializedWorld) {
    const { nextId, historicalLocations, currentArticleSummary, publicStatements } = serialized;
    ...
    this.currentArticleSummary = currentArticleSummary;
  ...
  serialize(): SerializedWorld {
    return {
      ...
      currentArticleSummary: this.currentArticleSummary,
